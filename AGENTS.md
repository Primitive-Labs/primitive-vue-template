...
- `/public`: Copied verbatim into `dist/` by Vite. `_headers` sets response headers on those assets (Cloudflare Workers static assets read it) — the one place for that, since `worker.js` is library-provided. `.well-known/apple-app-site-association` is what lets an iOS client of this same app open this domain's links: the emailed sign-in link is one `https://<domain>/oauth/callback` URL that opens the app where it is installed and signs in here otherwise. That document is NOT committed — `scripts/deploy.mjs` generates it before each build from the deployed Primitive environment's `iosAppId`, because an app has one bundle id per environment (`.well-known/apple-app-site-association.example` is only a reference copy). `.well-known/README.md` has the activation order — declare `iosAppId`, deploy, verify with `curl`, and only then enable the app's `applinks:` entitlement.
...
- Typed workflow invokers are generated from the `workflows/*.toml` schemas in the sync directory beside the project config (`<project root>/.primitive/sync/<env>/<appId>/workflows/`). `pnpm codegen` regenerates them into `src/types/generated/workflows/` whenever the SELECTED environment's app slot has workflow TOMLs — the same environment the CLI itself resolves, so a class synced only under another environment is skipped rather than generated from the wrong backend.
- **Generated code is code**: everything under `src/models/*.generated.ts` and `src/types/generated/` is committed and reviewed like any other source, and nothing generated is gitignored. `dev`, `build`, `test` and `cf-deploy` all run `pnpm codegen` first, so a schema change plus any of them shows the regenerated output as a working-tree diff you commit with the change. There is no drift check, no pre-commit hook and no CI step.
