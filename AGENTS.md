...
- Typed workflow invokers are generated from the `workflows/*.toml` schemas in the committed tree beside the project config (`<project root>/primitive/<env>/workflows/`). `pnpm codegen` regenerates them into `src/types/generated/workflows/` whenever the SELECTED environment has workflow TOMLs — the same environment the CLI itself resolves, so a class synced only under another environment is skipped rather than generated from the wrong backend.
